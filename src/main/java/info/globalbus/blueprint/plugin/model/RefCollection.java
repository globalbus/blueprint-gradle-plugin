...
class RefCollection implements XmlWriter {
    ...
    static RefCollection getRefCollection(BlueprintRegistry blueprintRegistry, Class<?> injectedType,
    ...
    private static List<String> getMatchingRefs(BlueprintRegistry blueprintRegistry, Annotation[] annotations) {
        for (CollectionDependencyAnnotationHandler<? extends Annotation> collectionDependencyAnnotationHandler :
            Handlers.COLLECTION_DEPENDENCY_ANNOTATION_HANDLERS) {
            Annotation annotation = AnnotationHelper.findAnnotation(annotations,
                collectionDependencyAnnotationHandler.getAnnotation());
            if (annotation != null) {
